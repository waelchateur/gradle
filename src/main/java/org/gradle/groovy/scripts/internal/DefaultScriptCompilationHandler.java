...
public class DefaultScriptCompilationHandler implements ScriptCompilationHandler {
  ...
  private static final NoOpGroovyResourceLoader NO_OP_GROOVY_RESOURCE_LOADER =
      new NoOpGroovyResourceLoader();
  ...
  public void compileToDir(
    ...
    try {
      compileScript(
          source, classLoader, configuration, metadataDir, extractingTransformer, verifier);
    } catch (Exception e) {
      try {
    ...
    try {
      try {
        ...
        throw new GradleException(
            String.format("Could not compile %s.", source.getDisplayName()), e);
      }
  ...
  private static class ClassesDirCompiledScript<T extends Script, M>
    ...
    public void onReuse() {
      if (scriptClass != null) {
        // Recreate the script scope and ClassLoader, so that things that use scopes are notified
    ...
    private ClassLoaderScope prepareClassLoaderScope() {
      String scopeName =
          "groovy-dsl:" + source.getFileName() + ":" + scriptBaseClass.getSimpleName();
      return targetScope.createLockedChild(
          scopeName,
          scriptClassPath,
          sourceHashCode,
  ...
  /**
   * A specialized ClassLoader that avoids unnecessary delegation to the parent ClassLoader, and the
   * resulting cascade of ClassNotFoundExceptions for those classes that are known to be available
   * only in this ClassLoader and nowhere else.
   */
  private static class ScriptClassLoader extends VisitableURLClassLoader
  ...
  private static class CustomScriptClassLoader extends DexBackedURLClassLoader
    ...
    CustomScriptClassLoader(
      ...
      super(
          ...
          classPath,
          implementationHash);
      classPath.getAsFiles().forEach(file -> compileJar(file.getAbsolutePath(), null));
      this.scriptSource = scriptSource;
      this.implementationHash = implementationHash;
    }
