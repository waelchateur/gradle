...
public class DefaultClassLoaderRegistry implements ClassLoaderRegistry {
  ...
  public DefaultClassLoaderRegistry(
    ...
    this.pluginsClassLoader =
        new MixInLegacyTypesClassLoader(
            runtimeClassLoader,
            classPathRegistry.getClassPath("GRADLE_EXTENSIONS"),
            legacyTypesSupport);
    this.gradleApiSpec = apiSpecFor(pluginsClassLoader);
    this.workerExtensionSpec =
        new MixInLegacyTypesClassLoader.Spec(
            "legacy-mixin-loader",
  ...
  private FilteringClassLoader.Spec apiSpecFor(ClassLoader classLoader) {
    ...
    GradleApiSpecProvider.Spec apiAggregate =
        new GradleApiSpecAggregator(classLoader, instantiator).aggregate();
