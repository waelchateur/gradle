...
public abstract class ClassLoaderUtils {
  ...
  static {
    ...
    CLASS_LOADER_PACKAGES_FETCHER =
        JavaVersion.current().isJava9Compatible()
            ? new LookupPackagesFetcher()
            : new ReflectionPackagesFetcher();
  }

  /**
   * Returns the ClassLoader that contains the Java platform classes only. This is different to
   * {@link ClassLoader#getSystemClassLoader()}, which includes the application classes in addition
   * to the platform classes.
   */
  ...
  public static void disableUrlConnectionCaching() {
    ...
    // URLConnection default caching should be disabled since it causes jar file locking issues and
    // JVM crashes in updating jar files.
    // Changes to jar files won't be noticed in all cases when caching is enabled.
    // sun.net.www.protocol.jar.JarURLConnection leaves the JarFile instance open if URLConnection
    // caching is enabled.
    try {
  ...
  public static <T> Class<T> define(
      ClassLoader targetClassLoader, String className, byte[] clazzBytes) {
  ...
  public static <T> Class<T> defineDecorator(
    ...
    return CLASS_DEFINER.defineDecoratorClass(
        decoratedClass, targetClassLoader, className, clazzBytes);
  }
   ...
   *
   * <p>On Java 8, the implementation is simply invoking {@link ClassLoader#defineClass}
   * reflectively.
   *
   * <p>Since Java 9, reflection is severely restrained, and a new API {@link
   * MethodHandles.Lookup#defineClass} is introduced. However, this API can only "defines a class to
   * the same class loader and in the same runtime package and protection domain as this lookup's
   * lookup class", which means, we can only use this API safely in the decorating scenario where
   * the decorated class acts as the lookup object.
   *
   * <p>Otherwise, we have to use {@link MethodHandle} to invoke {@link ClassLoader#defineClass}.
   * Fortunately, this is the rare case.
   */
  ...
  /**
   * This class makes it a bit easier to use {@link MethodHandles.Lookup}. In order to access a
   * method, a lookup object which is accessible to this method must be provided. Usually, this
   * class and the target Gradle-managed class loader exist in the same module, so everything works.
   * Otherwise, an {@link IllegalAccessException} will be thrown, and {@link ClassLoader} class will
   * be used as the lookup object.
   */
  private static class AbstractClassLoaderLookuper {
    ...
    protected <T> T invoke(
        ClassLoader classLoader, String methodName, MethodType methodType, Object... arguments) {
      try {
    ...
    private MethodHandles.Lookup getLookupForClassLoader(ClassLoader classLoader)
  ...
  private static class LookupClassDefiner extends AbstractClassLoaderLookuper
      implements ClassDefiner {
    private MethodType defineClassMethodType =
        MethodType.methodType(
            Class.class, new Class<?>[] {String.class, byte[].class, int.class, int.class});
  ...
  private static class LookupPackagesFetcher extends AbstractClassLoaderLookuper
      implements ClassLoaderPackagesFetcher {
    private MethodType getPackagesMethodType =
        MethodType.methodType(Package[].class, new Class<?>[] {});
    private MethodType getDefinedPackageMethodType =
        MethodType.methodType(Package.class, new Class<?>[] {String.class});
