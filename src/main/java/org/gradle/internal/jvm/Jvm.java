...
public class Jvm implements JavaInfo {
  ...
  private static final AtomicReference<JvmImplementation> CURRENT =
      new AtomicReference<JvmImplementation>();
  ...
  /**
   * Creates JVM instance for given java home. Attempts to validate if provided javaHome is a valid
   * jdk or jre location. This method is intended to be used for user supplied java homes.
   *
   * @param javaHome - location of your jdk or jre (jdk is safer), cannot be null
   * @return jvm for given java home
   * @throws org.gradle.internal.jvm.JavaHomeException when supplied javaHome does not seem to be a
   *     valid jdk or jre location
  ...
  /**
   * Creates JVM instance for given values. This method is intended to be used for discovered java
   * homes.
   */
  public static Jvm discovered(
      File javaHome, String implementationJavaVersion, JavaVersion javaVersion) {
  ...
  public String toString() {
    ...
    return SystemProperties.getInstance().getJavaVersion()
        + " ("
  ...
  public File getJavaExecutable() throws JavaHomeException {
    ...
    // deenu modify: add dalvikvm
    /** javaExecutable = findExecutable("java"); */
    javaExecutable = os.isAndroid() ? findExecutable("dalvikvm") : findExecutable("java");
  ...
  public File getJavaHome() {
    // deenu modify: return system path
    if (os.isAndroid()) {
      File dalvikvm = new File("/apex/com.android.art/bin/dalvikvm");
      if (dalvikvm.exists()) {
        return new File("/apex/com.android.art");
      } else {
  ...
  /**
   * Locates a stand-alone JRE installation for this JVM. Returns null if not found. This is the JRE
   * installed outside the JDK installation.
   */
  @Nullable
  ...
  /**
   * Locates the JRE installation contained within this JVM. Returns null if no JRE installation is
   * available.
   */
  @Nullable
  ...
  /**
   * Note: Implementation assumes that an Apple JVM always comes with a JDK rather than a JRE, but
   * this is likely an over-simplification.
   */
