...
public class DefaultJvmMetadataDetector implements JvmMetadataDetector {
  ...
  private JvmInstallationMetadata asMetadata(
      File javaHome, EnumMap<ProbedSystemProperty, String> metadata) {
    String implementationVersion =
        (isRunningAndroid() || isDalvik()) ? "1.8" : metadata.get(ProbedSystemProperty.VERSION);
  ...
  private JvmInstallationMetadata getMetadataFromInstallation(File jdkPath) {
    ...
    try {
      ...
      return failure(
          jdkPath,
          "Command returned unexpected result code: "
              + exitValue
              + "\nError output:\n"
              + errorOutput);
