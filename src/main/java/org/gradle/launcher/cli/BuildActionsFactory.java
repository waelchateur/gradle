...
class BuildActionsFactory implements CommandLineActionCreator {
  ...
  public Action<? super ExecutionListener> createAction(
      CommandLineParser parser, ParsedCommandLine commandLine) {
    Parameters parameters = parametersConverter.convert(commandLine, null);

    parameters
        .getDaemonParameters()
        .applyDefaultsFor(
            jvmVersionDetector.getJavaVersion(parameters.getDaemonParameters().getEffectiveJvm()));
    ...
    if (parameters.getDaemonParameters().isEnabled()) {
      return Actions.toAction(
          runBuildWithDaemon(parameters.getStartParameter(), parameters.getDaemonParameters()));
    }
    if (canUseCurrentProcess(parameters.getDaemonParameters())) {
      return Actions.toAction(
          runBuildInProcess(parameters.getStartParameter(), parameters.getDaemonParameters()));
    }

    return Actions.toAction(
        runBuildInSingleUseDaemon(
  ...
  private Runnable runBuildWithDaemon(
      StartParameterInternal startParameter, DaemonParameters daemonParameters) {
    ...
    return runBuildAndCloseServices(
        startParameter, daemonParameters, client, clientSharedServices, clientServices);
  }
  ...
  private Runnable runBuildInProcess(
    ...
    // Force the user home services to be stopped first, the dependencies between the user home
    // services and the global services are not preserved currently
    return runBuildAndCloseServices(
        startParameter,
        daemonParameters,
    ...
    ServiceRegistry clientServices =
        clientSharedServices
            ...
            .createSingleUseDaemonClientServices(
                clientSharedServices.get(OutputEventListener.class), daemonParameters, System.in);
    DaemonClient client = clientServices.get(DaemonClient.class);
    return runBuildAndCloseServices(
        startParameter, daemonParameters, client, clientSharedServices, clientServices);
  }
  ...
  private Runnable runBuildAndCloseServices(
    ...
    BuildActionParameters parameters =
        createBuildActionParameters(startParameter, daemonParameters);
    Stoppable stoppable =
  ...
  private long getBuildStartTime() {
    // deenu modify: add android check
    return (isRunningAndroid() || isDalvik())
