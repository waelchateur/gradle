...
public class ProcessBootstrap {
  /**
   * Sets up the ClassLoader structure for the given class, creates an instance and invokes {@link
   * EntryPoint#run(String[])} on it.
   */
  ...
  private static void runNoExit(String mainClassName, String[] args) throws Exception {
    ...
    ClassLoader antClassLoader =
        classLoaderFactory.createIsolatedClassLoader("ant-loader", antClasspath);
    ClassLoader runtimeClassLoader =
        new VisitableURLClassLoader("ant-and-gradle-loader", antClassLoader, runtimeClasspath);

    ClassLoader oldClassLoader = Thread.currentThread().getContextClassLoader();
    Thread.currentThread().setContextClassLoader(runtimeClassLoader);

    try {
      // deenu modfify: runtimeClassLoader classpath is empty so run mainClassName directly
      /**
       * Class<?> mainClass = runtimeClassLoader.loadClass(mainClassName); Object entryPoint =
       * mainClass.getConstructor().newInstance(); Method mainMethod = mainClass.getMethod("run",
       * String[].class); mainMethod.invoke(entryPoint, new Object[]{args});
       */
      if (mainClassName == Main.class.getName()) {
        new Main().run(args);
      } else {
