...
public class DefaultDaemonStarter implements DaemonStarter {
  ...
  public DaemonStartupInfo startDaemon(boolean singleUse) {
    ...
    FlushableEncoder encoder =
        new KryoBackedEncoder(new EncodedStream.EncodedOutput(buffer.getOutputStream()));
    try {
  ...
  private List<String> getPriorityArgs(DaemonParameters.Priority priority) {
    ...
    if (os.isUnix()) {
      ...
      return Arrays.asList(
          "cmd", "/C", "start", "\"Gradle build daemon\"", "/B", "/belownormal", "/WAIT");
    } else {
      return Collections.emptyList();
    }
  }
  ...
  private DaemonStartupInfo startProcess(
    ...
    try {
      ...
      // This factory should be injected but leaves non-daemon threads running when used from the
      // tooling API client
      @SuppressWarnings("deprecation")
      DefaultExecActionFactory execActionFactory = DefaultExecActionFactory.root(gradleUserHome);
      try {
        ExecHandle handle =
            new DaemonExecHandleBuilder()
                .build(args, workingDir, outputConsumer, stdInput, execActionFactory.newExec());
